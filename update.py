...
def update_tool_types(tool_types, id):
    ...
    for type in tool_types:
        if not session.query(db.tool_types).filter(db.tool_types.bio_id == id, db.tool_types.name == type).first():
...
def update_institutes(institutes, id):
    ...
    for institute in institutes_list:
        if not session.query(db.institutes).filter(db.institutes.bio_id == id, db.institutes.name == institute).first():
...
def update_topics(topics, id):
    ...
    for i, term in enumerate(topic_term):
        if not session.query(db.topics).filter(db.topics.bio_id == id, db.topics.term == term).first():
...
def update_functions(functions, id):
    ...
    for i, term in enumerate(function_term):
        if not session.query(db.functions).filter(db.functions.bio_id == id, db.functions.term == term).first():
...
def update_platforms(platforms, id):
    ...
    for platform in platforms:
        if not session.query(db.platforms).filter(db.platforms.bio_id == id, db.platforms.name == platform).first():
...
def update_input_output(functions, id, input_or_outut, table):
    ...
    for item in session.scalars(items_from_db):
        if item.term not in items:
            ...
            session.delete(item)
    for item in items:
        if not session.query(table).filter(table.bio_id == id, table.term == item).first():
...
def update_collection_ids(collection_ids, id):
    collection_ids_from_db = select(db.collection_ids).where(db.collection_ids.bio_id == id)
    ...
    for collection_id in collection_ids:
        if not session.query(db.collection_ids).filter(db.collection_ids.bio_id == id, db.collection_ids.coll_id == collection_id).first():
...
def update_elixir_platforms_nodes_communities(items, id, table):
    ...
    for item in session.scalars(items_from_db):
        if item.name not in items:
            ...
            session.delete(item)
    for item in items:
        if not session.query(table).filter(table.bio_id == id, table.name == item).first():
...
def update_years(doi, pmid):
    ...
    for i in range(1, number_of_pages + 1):
        ...
        for item in response['citationList']['citation']:
            ...
            if year == '2022':
                year_2022 += 1
    year_2022_from_db = select(db.years).where(db.years.doi == doi, db.years.year == '2022')
...
def update_publications_and_years(publications, id):
    ...
    for publication in publications:
        ...
        if not session.query(db.publications).filter(db.publications.bio_id == id, db.publications.doi == pub_doi).first():
            ...
            new_publication = db.publications(doi=pub_doi, bio_id=id, pmid=pmid, pmcid=pmcid, citations_source=citations_source, impact_factor=impact_factor, journal=journal, citation_count=citation_count)
            session.add(new_publication)
            continue
        publication_from_db = select(db.publications).where(db.publications.bio_id == id, db.publications.doi == pub_doi)
...
def update_tools():
    ...
    for tool in session.scalars(tools_from_db):
        response = requests.get(f'https://bio.tools/api/tool/?&biotoolsID=\"{tool.bio_id}\"&format=json').json()
        ...
        tool.documentation = response_list['documentation'][0]['url'] if response_list['documentation'] else ''
        update_collection_ids(response_list['collectionID'], id)
        update_elixir_platforms_nodes_communities(response_list['elixirPlatform'], id, db.elixir_platforms)
        update_elixir_platforms_nodes_communities(response_list['elixirNode'], id, db.elixir_nodes)
        update_elixir_platforms_nodes_communities(response_list['elixirCommunity'], id, db.elixir_communities)
        tool.citation_count, tool.impact_factor, tool.journals = update_publications_and_years(response_list['publication'], id)
        tool.url, tool.created_at, tool.updated_at, tool.forks, tool.contributions = update_github_info(response_list['link'])
